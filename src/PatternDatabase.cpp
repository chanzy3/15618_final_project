...
namespace busybin
{
   ...
   * than 15).
   */
  bool PatternDatabase::setNumMoves(const cube_t* cube, const uint8_t numMoves)
  {
   ...
   * @param cube A cube instance.
   */
  uint8_t PatternDatabase::getNumMoves(const cube_t* cube) const
  {
   ...
   * cube instance.
   */
  uint8_t PatternDatabase::getNumMovesEx(const cube_t* cube,
    const uint8_t boundHint, const uint8_t depthHint) const
  {
    return this->getNumMoves(this->getDatabaseIndex(cube));
  }
