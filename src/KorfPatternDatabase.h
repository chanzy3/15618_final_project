...
namespace busybin
{
  ...
  public:
    ...
    uint8_t getNumMoves(const cube_t* cube) const;
    uint8_t getNumMovesEx(const cube_t* cube,
      const uint8_t boundHint, const uint8_t depthHint) const;
    bool setNumMoves(const cube_t* cube, const uint8_t numMoves);
    bool isFull() const;
    void inflate();
    void reset();

    // All unimplemented.
    uint32_t getDatabaseIndex(const cube_t* cube) const;
